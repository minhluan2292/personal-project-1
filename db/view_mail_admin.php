	...
	foreach ($dbh->query($sql) as $row)
	{
		if($row["state"]==1)
		{
		 echo '<tr>';
                    ...
                    echo '<td><input onkeydown="document.getElementById("b'.$row["id"].'").click()" id="'.$row["id"].'" style="display:inline-block;width:70%;margin-right:10px;" type="text" class="form-control" value="' . $row["email"] . '"><button style="display:inline-block;" id="b'.$row["id"].'" type="button" class="btn btn-success" onclick="updatemail('.$row["id"].')">Update</button></td>';
                    echo '<td>' . $row["name"] . '</td>';
         echo '</tr>';
		}
...
?>
<script>
	function updatemail(a)
	{
		var email = document.getElementById(a).value;
	
		var url="db/updatemail.php?id="+a+"&mail="+email;
    var Httpreq = new XMLHttpRequest(); 
    Httpreq.open("GET",url,false);
    Httpreq.send(null);

		window.location.href = '?page=view2';
	}
</script>
